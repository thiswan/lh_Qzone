 ...
 *文字滚动的view
 */
@property(nonatomic,strong)wordRollView* wordView;
...
//    [self.view addSubview:label];
    NSString* text=@"时间知道越是平凡的陪伴 就越长久,哈哈哈哈哈哈哈哈";
    CGRect frame=CGRectMake(20, 110, self.view.bounds.size.width-40, 40);
    self.wordView=[[wordRollView alloc]initWithFrame:frame title:text TextColor:[UIColor redColor]];
...
{
    if (btn.tag-500==1) {
        yeardayViewController* vc=[[yeardayViewController alloc]init];
...
- (void)setDownOneBtnAnim:(UIButton *)btn
{
    ...
    [UIView animateWithDuration:0.6 animations:^{
        ...
        [self dismissViewControllerAnimated:NO completion:^{
            ...
            [self.wordView stoprollAnimation];
