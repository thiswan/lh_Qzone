...
#warning 单例保存数据成功（8.29 星期六）
            //NSDictionary *dict =[NSDictionary dictionaryWithObject:self.registerQQField.text forKey:@"username"];
            //NSDictionary *PwdDict =[NSDictionary dictionaryWithObject:self.registerQQPswField forKey:@"userPwd"];
            //[[user  shareduser]setUserInfo:dict];
           // [[user shareduser]setUserPwdInfo:PwdDict];
            user *u=[user shareduser];
            u.user =self.registerQQField.text;
            u.pwd=self.registerQQPswField.text;
...
//          ViewController* vc=[[ViewController alloc]init];
//          vc.userInfo=[[user shareduser] GetUserInfo];
//          NSString *username = [vc.userInfo objectForKey:@"username"];
//          NSLog(@"取得用户注册QQ号%@",username);
//          NSString* u=[NSString stringWithFormat:@"%@",username];
//          NSLog(@"注册前的用户QQ号:%@",u);
//          vc.qqNumField.text=u;
//             
        if (self.registerQQField.text.length!=0 && self.registerQQPswField.text.length!=0)
        {
            NSLog(@"用户注册的号码密码不为空才需要将值传回登录控制器");
            AppDelegate *app = (AppDelegate *)[UIApplication sharedApplication].delegate;
            ViewController *viewCtrl = (ViewController *)app.window.rootViewController;
            
            
            viewCtrl.qqNumField.text = [user shareduser].user;
            viewCtrl.qqField.text=[user shareduser].pwd;
            viewCtrl.btn.hidden=NO;
#warning 这里下拉菜单按钮还原起初位置的原因是，model回登录控制器的时候，老是会向上移动十几个像素
            viewCtrl.btn.frame=CGRectMake(282, 220, 14, 9);
            [[user shareduser] saveUserInfoToSanbox];
        }
        else
        {
            NSLog(@"用户没有注册，直接回到了登陆界面");
        }
    }];
}
/**
